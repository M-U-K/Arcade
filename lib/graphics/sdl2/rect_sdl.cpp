...
#include "rect_sdl.hpp"

arcade::Rect::Rect(arcade::Scene *scene, std::pair<float, float> size)
{
    this->rect->w = size.first;
    this->rect->h = size.second; 
    this->load = ON_L;
}
...
arcade::Rect *arcade::create_rect(arcade::Scene *scene, float width, float height)
{
    ...
    arcade::Rect *rect = new arcade::Rect(scene, size);
    scene->add_rect(rect);
    return rect;
}
...
void arcade::Rect::set_rect_position(std::pair<float, float> size)
{
    this->rect->x = size.first;
    this->rect->y = size.second;
}
